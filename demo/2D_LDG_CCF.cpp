...
int main() {
    ...
    int save_time_num = 4;
    double save_time[save_time_num] = {0, 1, 2, 3.5};

    dle2Solver->computeTimeDiscretization(total_time, save_time, save_time_num);

    string filename0 = "0s_result.vtk";
    string filename1 = "1s_result.vtk";           
    string filename2 = "2s_result.vtk";   
    string filename3 = "3.5s_result.vtk";                                                                                 
    dle2Solver->outputVTKDataFile(write_path + filename0, 0);
    dle2Solver->outputVTKDataFile(write_path + filename1, 1);
    dle2Solver->outputVTKDataFile(write_path + filename2, 2);
    dle2Solver->outputVTKDataFile(write_path + filename3, 3.5);
